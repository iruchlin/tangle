...
for r in range(1, size):
    ...
    index_list[r, 1] = index_list[r, 0] + len_prefix_list // size + (1 if r < len_prefix_list % size else 0)

win_rem_list = MPI.Win.Allocate_shared(2 * len_rem_list * MPI.UINT64_T.Get_size(), MPI.UINT64_T.Get_size(), comm=comm)
win_prefix_list = MPI.Win.Allocate_shared(2 * len_prefix_list * MPI.UINT64_T.Get_size(), MPI.UINT64_T.Get_size(), comm=comm)

rem_buf, _ = win_rem_list.Shared_query(root)
rem_list = np.ndarray(buffer=rem_buf, dtype=np.uint64, shape=(len_rem_list, 2))

prefix_buf, _ = win_prefix_list.Shared_query(root)
prefix_list = np.ndarray(buffer=prefix_buf, dtype=np.uint64, shape=(len_prefix_list, 2))
...
prefix_list[index_list[rank, 0]:index_list[rank, 1]] = prefix_list[index_list[rank, 0]:index_list[rank, 1]][np.argsort(prefix_list[index_list[rank, 0]:index_list[rank, 1], 1], kind='stable')]
...
start_time = time.time()
t_count = count_tangles(n, rem_list, prefix_list[index_list[rank, 0]:index_list[rank, 1]])
print(f"rank {rank}, index [{index_list[rank, 0]}, {index_list[rank, 1]}], tangles {t_count}, time {time.time() - start_time}")
